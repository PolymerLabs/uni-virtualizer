...
class Layout1dBase {
    ...
    _updateVisibleIndices() {
        ...
        for (let i = this._first; i <= this._last; i++) {
            const itemPosition = this._getItemPosition(i)[this._positionDim];
            const itemSize = this._getItemSize(i)[this._sizeDim];
            // Several items may have the same position (in a grid layout, for example),
            // so only set the first visible index once.
            if (itemPosition + itemSize >= this._scrollPosition && firstVisible === -1) {
                firstVisible = i;
            }
            if (itemPosition < this._scrollPosition + this._viewDim1) {
                lastVisible = i;
            }
        }
        if (firstVisible === -1) {
...
export { Layout1dBase as L, Layout1d as a };
