import { L as Layout1dBase } from './chunk-24977e13.js';
...
class VirtualScroller extends VirtualRepeater {
    ...
    async _render() {
        if (this._lazyLoadDefaultLayout && !this._layout) {
            ...
            const { Layout1d } = await import('./Layout1d-38628aae.js');
...
const example = (contacts) => html`
    <section>
        ${scroll({
            items: contacts,
            renderItem: ({ first, last, color }, i) => html`<div style="background: ${color}; width: 100px; height: 80px; padding: 10px;">${i}<br>${first}<br>${last}</div>`,
            scrollTarget: window,
            layout: new Layout1dGrid({}),
        })}
    </section>
`;
