...
class Layout1dBase {
    ...
    _scrollIfNeeded() {
        // console.log("hii scrollToIndex is", this._scrollToIndex)
        // debugger
        if (this._scrollToIndex === -1) {
            return;
        }
        // Fix scroll start position. is it based on estimates...
...
class Layout1d extends Layout1dBase {
    ...
    _getItems(lower, upper) {
        ...
        if (this._stable) {
            // let's see if physicalItems and newPhysicalItems are exactly the same
            let done = true;
            if (this._newPhysicalItems.size === this._physicalItems.size) {
                for (let key of this._physicalItems.keys()) {
                    const p = this._physicalItems.get(key);
                    const n = this._newPhysicalItems.get(key);
                    // found a non-shared key-value pair
                    if (!(p && n && p.pos === n.pos && p.size === n.size)) {
                        done = false;
                    }
                }
            }
            ...
            if (done) {
                console.log("iz done", this._physicalItems);
            }
            else {
                console.log("iz not done");
            }
            this._newPhysicalItems = this._physicalItems;
            this._newPhysicalItems.clear();
            this._physicalItems = items;
