...
class Layout1dBase {
    constructor(config) {
        ...
        this._resetScrollToIndex = false;
    ...
    set viewportScroll(coords) {
        ...
        // console.log("setting viewportScroll to", this._scrollPosition)
    ...
    /**
     * Scroll to the child at the given index, and the given position within the
     * child.
     */
    scrollToIndex(index, position = 'start') {
        // console.log("called scrollToIndex with ", index)
        ...
        this._scheduleReflow();
        // remove this cause we need to control when we reflow (from render)
        // this.reflowIfNeeded();
    }
    ...
    _reflow() {
        // console.log("i better never get called or wtf")
        ...
        if (this._first === -1 && this._last === -1) {
            // TODO: have default empty object for  instead
            this._emitRange();
        }
    ...
    _scrollIfNeeded() {
        console.log("scrollIfNeeded. today we are scrolling to", this._scrollToIndex);
        // debugger
        ...
        // console.log("position of item", index, "is", pos)
        ...
        // if (curAnchorPos === newAnchorPos) {
        //   console.log("okeedoo, resetting scrollToIndex cause error is 0")
        //   this._scrollToIndex = -1;
        // } else {
        //   console.log("scroll error is not 0 yet...! uwu")
        // }
    ...
    _emitRange(inProps = undefined) {
        ...
        console.log('emitting range', detail);
    ...
    _checkThresholds() {
        if (this._viewDim1 === 0 && this._num > 0) {
            ...
            if (this._physicalMin > min || this._physicalMax < max) {
                console.log("scheduling a reflow cause someone scrolled");
                this._scheduleReflow();
            }
        }
    }
...
class Layout1d extends Layout1dBase {
    ...
    /**
     * Save the sizes of each measured child, including their margins in the
     * scrolling direciton.
     * Save the average size of the measured children for use in estimations.
     */
    updateItemSizes(sizes) {
        // debugger
        const _prevTMeasured = this._tMeasured;
        Object.keys(sizes).forEach((key) => {
            ...
            if (item) {
                let delta;
                if (size !== undefined) {
                    // Save the actual measured size of the item
                    item.size = size;
        ...
        // console.log("done updating sizes for", Object.keys(sizes))
        // if (this._physicalItems.has(this._scrollToIndex) && this._tMeasured === _prevTMeasured) {
        //   debugger
        //   console.log("we did it boiz")
        //   this._scrollToIndex = -1
        // }
        if (this._tMeasured === _prevTMeasured) ;
        if (!this._nMeasured) {
     ...
     * Estimates if the item at idx hasn't been measured yet.
     */
    _getPosition(idx) {
    ...
    _reflow() {
        ...
        this._getActiveItems();
        this._scrollIfNeeded();
        // console.log("..measured item in Q?", this._physicalItems.has(this._scrollToIndex))
        if (this._scrollSize !== _scrollSize) {
            this._emitScrollSize();
        }
        this._emitRange();
        if (this._first === -1 && this._last === -1) {
            ...
            // console.log("..scroll error", this._scrollError);
            this._emitChildPositions();
            this._emitScrollError();
            this._resetReflowState();
            // If we got a measurement for the scrollToIndex during the last reflow,
            // we can safely unset _scrollToIndex.
            // if (this._physicalItems.has(this._scrollToIndex)) {
            //   debugger
            //   this._resetScrollToIndex = true
            // }
        }
    }
    _resetReflowState() {
    ...
    /**
     * Returns the height and width of the item at idx, or estimates it.
     */
    _getItemSize(idx) {
        return {
    ...
    _scrollIfNeeded() {
        super._scrollIfNeeded();
        // If we had a measurement for the scrollToIndex during the last reflow,
        // we can safely unset _scrollToIndex.
        // if (this._physicalItems.has(this._scrollToIndex)) {
        //   this._scrollToIndex = -1
        // }
        // console.log(this._scrollPosition)
        // const haveMeasuredItemScrollToIndex = this._physicalItems.has(this._scrollToIndex)
        // const haveScrolledToItemPosition = this._scrollPosition === this._getItemPosition(this._scrollToIndex)[this._positionDim]
        // if (haveMeasuredItemScrollToIndex && haveScrolledToItemPosition) {
        //   console.log('uggggggghhhh')
        //   this._scrollToIndex = -1
        // }
        // if (this._resetScrollToIndex) {
        //   console.log('resetting the scroll-to index')
        //   this._scrollToIndex = -1
        // }
    }
}
