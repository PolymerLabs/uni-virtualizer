...
class Layout1dBase {
    ...
    /**
     * Emit a visibleindiceschange event with the indices of the first and
     * last items to intersect the viewport.
     */
    _emitVisibleIndices() {
        const detail = {
        ...
        this.dispatchEvent(new CustomEvent('visibleindiceschange', { detail }));
    }
    /**
    ...
    _updateVisibleIndices() {
        let firstVisibleIndex = this._firstVisibleIndex;
        ...
        if (firstVisibleIndex !== this._firstVisibleIndex || lastVisibleIndex !== this._lastVisibleIndex) {
            this._firstVisibleIndex = firstVisibleIndex;
            this._lastVisibleIndex = lastVisibleIndex;
            this._emitVisibleIndices();
        }
    }
...
class Layout1d extends Layout1dBase {
    ...
    _reflow() {
        ...
        this._emitRange();
        // Ensure the initialvisible indices are dispatched.
        this._updateVisibleIndices();
        if (this._first === -1 && this._last === -1) {
