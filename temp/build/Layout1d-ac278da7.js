...
class Layout1dBase {
    ...
    set viewportScroll(coords) {
        ...
        // THIS IS TRUE
        console.log("old pos", oldPos, "new pos", this._scrollPosition);
        if (oldPos !== this._scrollPosition) {
            console.log("👎 Should only see this if manually scrolled.");
    ...
    _scrollIfNeeded() {
        ...
        const scrollPosition = Math.floor(Math.min(this._scrollSize - this._viewDim1, Math.max(0, this._scrollPosition - curAnchorPos + newAnchorPos - 48)));
        this._scrollError += this._scrollPosition - scrollPosition;
        console.log("* determined scroll position to be", scrollPosition);
